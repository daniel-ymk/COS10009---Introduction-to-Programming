...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    while (column_index < x_cell_count) 
      row = Array.new(y_cell_count) #= 10
      @columns[column_index] = row  # 
      row_index = 0
      while (row_index < y_cell_count) #=10
        cell = Cell.new()
    ...
    row_index = 0
    while (row_index < x_cell_count)
    column_index = 0
      while column_index < y_cell_count
        print("Cell x: #{row_index}, y: #{column_index} ")
        if column_index -1 < 0
          cell.north = 0
        else 
        cell.north = 1
      ...
      puts ("north: #{cell.north} south: #{cell.south} east: #{cell.east} west: #{cell.west}")
      column_index += 1
      end
      puts ("--------------- End of Column --------------- \n")
    row_index += 1
