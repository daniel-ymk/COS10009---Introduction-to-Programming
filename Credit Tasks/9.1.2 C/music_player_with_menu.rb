...
#Menu Option 1, to read in an external file
def read_in_albums()
...
#Menu Option 2, Display all albums
...
#Submenu inside Option 2 to display albums
...
#Menu Option 3, Prompting the user to play a song
...
#read and return all the albums
...
#Display Albums by Genre inside submenu
...
def add_album(albums)
	...
	new_file = File.new('albums_new.txt', 'w')	
	album_title = read_string("Enter title: ")
	new_file.puts(album_title)
	new_file.puts(read_string("Enter artist: "))
	new_file.puts(read_string("Enter label: "))

	puts("1. Pop\n2. Classic\n3. Jazz\n4. Rock\n")
	new_file.puts(read_integer_in_range("Enter number of genre: ",1,4))

	count_tracks = check_integer("Enter number of tracks: ")
	new_file.puts(count_tracks.to_s)
	i = 0
	while i < count_tracks 
		new_file.puts(read_string("Enter a name for the new track"))
		new_file.puts(read_string("Enter a location for the new track"))
		i += 1
	end
	new_file.close

	read_file = File.new(new_file, "r")
	puts("Album added: #{album_title}. Press enter to continue.")
	new_album = read_album(read_file)
	albums << new_album
	$album_read == true
	read_file.close
	return albums
